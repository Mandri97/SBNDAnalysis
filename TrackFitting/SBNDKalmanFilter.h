 ...
 */
 ...
 /**
  *  Kalman filter originally written for LArContent by Andy Chappell
  *  Ported and modified to work with SBND by Manoa Andriamirado 
  */
#ifndef SBND_KALMAN_FILTER_H
#define SBND_KALMAN_FILTER_H
...
public:
    ...
    // constructor overloading with SBND recob::tracking::Point_t type
    KalmanFilter(double dt, double processVariance, double measurementVariance, const recob::tracking::Point_t &x, const double initialCovariance = 1.0);
     ...
     *  @param  z The position to use for updating the state estimate
     */
    void Update(const recob::tracking::Point_t &z);
     ...
     * @return The temporary state vector
     */
    Eigen::Matrix<double, DIM, 1> ConvertToEigenMatrix(const recob::tracking::Point_t &p) const;

    // I might have to convert back to root position as well
...
template <int DIM>
KalmanFilter<DIM>::KalmanFilter(double dt, double processVariance, double measurementVariance, const recob::tracking::Point_t &x,
    const double initialCovariance) :
    KalmanFilter<DIM>::KalmanFilter(
        dt, 
        processVariance, 
        measurementVariance,
        ConvertToEigenMatrix(x),
        initialCovariance) { }
...
template <int DIM>
void KalmanFilter<DIM>::Update(const recob::tracking::Point_t &z)
{
    Update(ConvertToEigenMatrix(z));
}
...
template <int DIM>
Eigen::Matrix<double, DIM, 1> KalmanFilter<DIM>::ConvertToEigenMatrix(const recob::tracking::Point_t &p) const
{
    static_assert(DIM != 3, "Dimension error! Use Eigen::Matrix<T, NROW, 1> for NROW != 3 and NCOL != 1");

    return Eigen::Matrix<double, DIM, 1>(p.x(), p.y(), p.z());
}
...
#endif // #ifndef SBND_KALMAN_FILTER